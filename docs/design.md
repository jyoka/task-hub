...
- カードの取得は最大 200 件で、超えても警告しません(推測)。開いているカードが 200 件を超えると、見えないカードを
  Done とみなして worktree を消すおそれがあります。Done が 200 件を超えると `task show` などで古い番号が見つからなくなります。
- `gh project item-list --query` は新しめの `gh` にしかありません(推測)。古い `gh` では毎回の確認が失敗します。
