---
name: task
description: Register the current conversation as a draft task on the user's task-hub board. Use ONLY when the user explicitly invokes this skill (for example /task) or explicitly asks to register or save this as a task. Never use it on your own initiative.
...
2. Write the goal to a temporary file and run:
   ...
   ```
   task new --title "<title>" --repo <owner/name> [--agent <agent>] [--theme <theme>] --goal-file <tmpfile>
   ```

   If `task` is not on PATH, use `"$HOME/AIprogramming PJ/task-hub/bin/task"`.

3. Reply with the task id, its file path, and one line: "Run `task start` when you want an agent to pick it up."
